...
public class UserController {
    ...
    public Response createUser(User user) throws
        ...
        try{
            ...
            String rib = UUID.randomUUID().toString().replace("-", "");
            accountDAO.save(new Account(null,user,rib,user.getFullName(),0,4000,4000));
    ...
    public Response editInfos (User user, @CookieParam("access_token") Cookie cookie) {
        try {
            ...
            Account fullAccount = accountDAO.findByUserId(u.getId()).get(0);

            MessageProducer producer = new MessageProducer();
            producer.publishUserAuthInfo(u, fullAccount);
